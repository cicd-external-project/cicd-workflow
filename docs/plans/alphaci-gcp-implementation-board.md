...
| Local Postgres/Supabase migration apply check needs a disposable database | Backend owner | SQL migration verifier now fails safely without a URL and masks DB targets, but it has not yet been applied to a real disposable database. | Use `npm run db:verify:gcp-runtime-migration` with `GCP_RUNTIME_MIGRATION_VERIFY_DATABASE_URL` once a local/shadow DB is available; keep shared/staging promotion blocked until apply/rollback and table checks pass. | Migration and rollback are applied successfully against a Supabase shadow/local database. |
