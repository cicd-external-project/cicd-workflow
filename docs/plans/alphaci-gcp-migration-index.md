...
| 0B | Access-independent local implementation queue | `docs/plans/gcp/11-access-independent-local-implementation.md` | Reviewed | Local backend, workflow, frontend, database-verifier, admin, and board work completed; cloud-repo branch alignment, disposable DB apply, and live GCP access remain blocked |
...
| 3 | Database expand-contract migration | `docs/plans/gcp/02-database-expand-contract.md` | In progress | Runtime metadata migration and rollback exist; verifier now fails safely, masks DB URLs, and checks runtime tables; disposable DB apply/rollback evidence remains |
...
| Blocker | Owner | Type | Blocked live command | Safe work while waiting | Clear condition | Verification after access |
| --- | --- | --- | --- | --- | --- | --- |
| GitHub private-repo branch protection unavailable for `alphaexplora-cloud` on the current GitHub plan | GitHub org owner | External GitHub plan limit | GitHub required-review/ruleset enforcement on private `alphaexplora-cloud` `main` | Keep repo private, squash-only, manual review, CODEOWNERS, no direct Terraform apply, and CI checks before merge | GitHub plan/ruleset support allows private repo branch protection, or repo visibility/plan changes are approved | Confirm repository rulesets/branch protection from GitHub settings or `gh api repos/<org>/alphaexplora-cloud/rulesets` |
| GCP account reauthentication required for `abtorres.it@alphaexplora.com` | GCP account owner | Operator auth | `gcloud organizations list --format=json`, API enablement, WIF creation, service account creation, and smoke deploys | Continue docs, IaC static checks, DB migrations, backend tests, workflow contract tests, and access requests | `gcloud auth login abtorres.it@alphaexplora.com` and `gcloud auth application-default login` complete successfully | `gcloud auth list --filter=status:ACTIVE --format=json` |
| GCP organization/folder permissions not granted yet | GCP organization admin | GCP IAM/access | `terraform -chdir=infra/gcp/foundation plan`, folder creation, org IAM review, project factory, billing-link automation, and dedicated-customer project placement | Keep Terraform foundation dry-run/static, maintain the IAM matrix, and build app/backend code behind flags | Required roles from `docs/plans/gcp/gcp-iam-access-request-matrix.md` are granted and verified | `gcloud resource-manager folders list --organization=<ORG_ID> --format=json` |
| Postgres/Supabase migration apply verification needs a disposable database URL | Backend owner | Local tooling/test environment | `npm run db:verify:gcp-runtime-migration` with a disposable DB URL | Verifier fails safely without a URL and masks DB targets; keep migration expand-only and defer shared/staging DB promotion | Migration and rollback apply cleanly against a Supabase shadow/local database | `npm run db:verify:gcp-runtime-migration` with `GCP_RUNTIME_MIGRATION_VERIFY_DATABASE_URL` set to a local/shadow DB |
| `alphaexplora-cloud` is not currently on `feature/migrate-vercel-render-to-gcp` | Foundation owner | Branch governance | Cloud-repo static hardening commits under the never-leave-feature-branch rule | Continue AlphaCI backend, frontend, workflow, database, and board work on the feature branch; do not edit `alphaexplora-cloud` until branch alignment is explicit | `alphaexplora-cloud` is confirmed or created on `feature/migrate-vercel-render-to-gcp` before cloud-repo edits | `git -C C:\Codes\cicd-ex\alphaexplora-cloud branch --show-current` |
| actionlint unavailable locally | Workflow owner | Local tooling/test environment | `actionlint` verification of changed workflow templates | Use `node scripts\validate-gcp-cloud-run-workflow.cjs`, JSON parsing checks, forbidden-pattern scans, and `git diff --check`; do not download/execute actionlint without explicit approval | actionlint is installed locally, or user explicitly approves a temporary actionlint download and execution | `actionlint` from `C:\Codes\cicd-ex\cicd-workflow` |
## Split Rule
