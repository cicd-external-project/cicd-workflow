...
| Postgres/Supabase migration apply verification needs a disposable database URL | Local tooling/test environment | Promoting DB migration beyond local code review | Verifier fails safely without a URL and masks DB targets; run `npm run db:verify:gcp-runtime-migration` with `GCP_RUNTIME_MIGRATION_VERIFY_DATABASE_URL` when a local/shadow DB is available; defer shared/staging DB promotion until then | Migration and rollback apply cleanly against a Supabase shadow/local database |
| `alphaexplora-cloud` is not currently on `feature/migrate-vercel-render-to-gcp` | Branch governance | Cloud-repo static hardening under the never-leave-feature-branch rule | Continue AlphaCI backend/frontend/workflow/database work on the feature branch; do not edit `alphaexplora-cloud` until branch alignment is explicit | `alphaexplora-cloud` is confirmed or created on `feature/migrate-vercel-render-to-gcp` before cloud-repo edits |
| actionlint unavailable locally | Local tooling/test environment | Actionlint verification of changed workflow templates | Use `node scripts\validate-gcp-cloud-run-workflow.cjs`, JSON parsing checks, forbidden-pattern scans, and `git diff --check`; do not download/execute actionlint without explicit approval | actionlint is installed locally, or user explicitly approves a temporary actionlint download and execution |
